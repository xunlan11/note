...
\item 填充：$ P $。
\item 步幅：$ S $。
\item 新图像大小：$ N_1 = \frac{N_0 + 2P - F}{S} + 1, \quad A_1 = B $。
\item 参数量：$ F \times F \times A_0 \times B $。
\end{itemize}

$ 1 \times 1 $卷积\tip{$ 1 \times 1 $卷积的作用}：
\begin{itemize}
...
%------------------------------------------------
\subsubsection[损失函数]{损失函数\tip{交叉熵损失函数}}
交叉熵损失函数更适用于分类问题，常用于衡量模型预测的概率分布与真实标记的概率分布之间的差异。
%------------------------------------------------
\subsubsection[激活函数]{激活函数\tip{激活函数}}
\begin{table}[H]
\centering
...
%------------------------------------------------
\subsubsection[LSTM]{LSTM（Long Short-Term Memory networks，长短期记忆网络）\tip{LSTM}}
...
\end{itemize}
%------------------------------------------------
\paragraph{多头自注意力（Multi-Head Self-Attention）}\tip{多头自注意力}
\begin{enumerate}
